...
// 2. Set env vars: NEXT_PUBLIC_DODO_PUBLIC_KEY, NEXT_PUBLIC_DODO_BASIC_PRODUCT_ID,
//    NEXT_PUBLIC_DODO_PROFESSIONAL_PRODUCT_ID, NEXT_PUBLIC_DODO_ENTERPRISE_PRODUCT_ID
...
const planConfigs = {
  basic: {
    ...
    productIdKey: 'NEXT_PUBLIC_DODO_BASIC_PRODUCT_ID',
  ...
  professional: {
    ...
    productIdKey: 'NEXT_PUBLIC_DODO_PROFESSIONAL_PRODUCT_ID',
  ...
  enterprise: {
    ...
    productIdKey: 'NEXT_PUBLIC_DODO_ENTERPRISE_PRODUCT_ID',
    productIdFallback: 'dodo_enterprise_placeholder'
  }
};
function getProductId(_key: string, fallback: string): string {
  return fallback;
}
function CheckoutContent() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(`/register?plan=${planKey}&payment_ref=demo_ref`);
      }, 1200);
    } catch (err) {
    ...
    <div className="max-w-lg mx-auto px-4 sm:px-6 pt-36 pb-24">
      <motion.div
        ...
        <div className="flex items-center gap-2 text-slate-500 text-xs mb-8">
          ...
          <span>
            Secure checkout powered by DodoPayments. Your payment info is never
            stored.
          </span>
        </div>
