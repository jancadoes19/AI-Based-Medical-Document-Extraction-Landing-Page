import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { Linkedin, Instagram } from 'lucide-react';
import { CookieBanner } from './CookieBanner';
const NEW_LOGO = "/Image_20-02-26_at_14.22.png";

export function Footer() {
  const [showCookieBanner, setShowCookieBanner] = useState(false);
  return (
    <footer className="bg-slate-50 border-t border-slate-200 pt-20 pb-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 mb-16">
          {/* Column 1: Logo & Description */}
          <div className="col-span-2 md:col-span-1">
            <div className="mb-5 h-[44px] flex items-center">
              <Link to="/" className="block">
                <img src={NEW_LOGO} alt="Immenzo" className="h-11 w-auto" />
              </Link>
            </div>
            <p className="text-slate-500 text-sm leading-relaxed">
              Intelligent document automation for enterprises in Indonesia.
            </p>
          </div>

          {/* Column 2: Company */}
          <div>
            <h4 className="font-bold text-slate-900 mb-5 text-xs uppercase tracking-widest h-[44px] flex items-center">
              Company
            </h4>
            <ul className="space-y-3 text-sm text-slate-600">
              <li>
                <Link
                  to="/about"
                  className="hover:text-[#063aa4] transition-colors">

                  About Us
                </Link>
              </li>
              <li>
                <Link
                  to="/manifesto"
                  className="hover:text-[#063aa4] transition-colors">

                  Manifesto
                </Link>
              </li>
              <li>
                <Link
                  to="/ai-ethics"
                  className="hover:text-[#063aa4] transition-colors">

                  AI Ethics
                </Link>
              </li>
              <li>
                <Link
                  to="/careers"
                  className="hover:text-[#063aa4] transition-colors">

                  Careers
                </Link>
              </li>
              <li>
                <a
                  href="mailto:support@immenzo.com"
                  className="hover:text-[#063aa4] transition-colors">

                  Contact
                </a>
              </li>
            </ul>
          </div>

          {/* Column 3: Product */}
          <div>
            <h4 className="font-bold text-slate-900 mb-5 text-xs uppercase tracking-widest h-[44px] flex items-center">
              Product
            </h4>
            <ul className="space-y-3 text-sm text-slate-600">
              <li>
                <Link
                  to="/immenzo-ai"
                  className="hover:text-[#063aa4] transition-colors">

                  Immenzo AI
                </Link>
              </li>
              <li>
                <Link
                  to="/use-cases"
                  className="hover:text-[#063aa4] transition-colors">

                  Use Cases
                </Link>
              </li>
              <li>
                <Link
                  to="/integrations"
                  className="hover:text-[#063aa4] transition-colors">

                  Integrations
                </Link>
              </li>
              <li>
                <Link
                  to="/pricing"
                  className="hover:text-[#063aa4] transition-colors">

                  Pricing & FAQ
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 4: Resources */}
          <div>
            <h4 className="font-bold text-slate-900 mb-5 text-xs uppercase tracking-widest h-[44px] flex items-center">
              Resources
            </h4>
            <ul className="space-y-3 text-sm text-slate-600">
              <li>
                <Link
                  to="/portal?redirect=/immenzo-ai"
                  className="hover:text-[#063aa4] transition-colors">

                  Documentation
                </Link>
                <span className="block text-[10px] text-slate-400 mt-0.5">
                  Available upon request
                </span>
              </li>
              <li>
                <Link
                  to="/portal?redirect=/immenzo-ai"
                  className="hover:text-[#063aa4] transition-colors">

                  Demo
                </Link>
                <span className="block text-[10px] text-slate-400 mt-0.5">
                  Available upon request
                </span>
              </li>
              <li>
                <Link
                  to="/blog"
                  className="hover:text-[#063aa4] transition-colors">

                  Blog
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 5: Connect */}
          <div>
            <h4 className="font-bold text-slate-900 mb-5 text-xs uppercase tracking-widest h-[44px] flex items-center">
              Connect
            </h4>
            <div className="flex flex-col gap-3">
              <a
                href="https://linkedin.com"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-slate-600 hover:text-[#063aa4] transition-colors text-sm">

                <Linkedin className="w-4 h-4" /> LinkedIn
              </a>
              <a
                href="https://instagram.com"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-slate-600 hover:text-[#063aa4] transition-colors text-sm">

                <Instagram className="w-4 h-4" /> Instagram
              </a>
              <a
                href="https://wa.me/6281234567890"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-slate-600 hover:text-[#063aa4] transition-colors text-sm">

                {/* WhatsApp icon */}
                <svg
                  className="w-4 h-4"
                  viewBox="0 0 24 24"
                  fill="currentColor">

                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                WhatsApp
              </a>
            </div>
          </div>

          {/* Column 6: Legal */}
          <div>
            <h4 className="font-bold text-slate-900 mb-5 text-xs uppercase tracking-widest h-[44px] flex items-center">
              Legal
            </h4>
            <ul className="space-y-3 text-sm text-slate-600">
              <li>
                <Link
                  to="/legal"
                  className="hover:text-[#063aa4] transition-colors">

                  Sub-Processors
                </Link>
              </li>
              <li>
                <Link
                  to="/legal"
                  className="hover:text-[#063aa4] transition-colors">

                  Privacy Policy
                </Link>
              </li>
              <li>
                <Link
                  to="/legal"
                  className="hover:text-[#063aa4] transition-colors">

                  Cookie Policy
                </Link>
              </li>
              <li>
                <Link
                  to="/legal"
                  className="hover:text-[#063aa4] transition-colors">

                  Terms of Service
                </Link>
              </li>
              <li>
                <Link
                  to="/security"
                  className="hover:text-[#063aa4] transition-colors">

                  Security
                </Link>
              </li>
              <li>
                <button
                  onClick={() => setShowCookieBanner(true)}
                  className="hover:text-[#063aa4] transition-colors text-left">

                  Cookie Settings
                </button>
              </li>
            </ul>
          </div>
        </div>

        {/* Copyright */}
        <div className="border-t border-slate-200 pt-8 flex flex-col md:flex-row justify-between items-center text-sm text-slate-500">
          <p>
            &copy; {new Date().getFullYear()} Immenzo by PT Immenzo Jaya
            International. All rights reserved.
          </p>
        </div>
      </div>

      <CookieBanner
        isOpen={showCookieBanner}
        onClose={() => setShowCookieBanner(false)}
        onAcceptAll={() => {
          localStorage.setItem('immenzo_cookie_consent', 'all');
          setShowCookieBanner(false);
        }}
        onEssentialOnly={() => {
          localStorage.setItem('immenzo_cookie_consent', 'essential');
          setShowCookieBanner(false);
        }} />

    </footer>);

}